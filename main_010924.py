    ...
    def tilt_baseline(self):
        ...
        for i in range(2):
            ...
            for folder_name in folders:
                ...
                faint_ind = np.argmin(np.abs(self.imp1.timestamps - self.imp1.t_end))
                faint_ind_hrv = np.argmin(np.abs(self.bpecg.hrtime - self.imp1.t_end))
                faint_ind_map = np.argmin(np.abs(self.bpecg.maptime - self.imp1.t_end))
                faint_ind_sys = np.argmin(np.abs(self.bpecg.systime - self.imp1.t_end))
                faint_ind_dia = np.argmin(np.abs(self.bpecg.diatime - self.imp1.t_end))
                comparing_method = 'max'
            ...
            if i == 0:
                ...
                self.rel_imp_df_pos.to_excel(excel_writer = "D:\Impedance_Measurements\Data/relative impedance max change pos.xlsx")
            if i == 1:
                ...
                self.rel_imp_df_neg.to_excel(excel_writer="D:\Impedance_Measurements\Data/relative impedance max change neg.xlsx")
